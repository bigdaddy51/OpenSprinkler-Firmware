 ...
 * <http://www.gnu.org/licenses/>.iioipoifgipfigpodifpgidpoifgpidpfgipdifg
 */
...
void flow_poll() {
	...
	// Hardware sensor reading (original implementation)
...
void check_flowdata() {
	...
	if (!flowapi_lasttime || (ntz > flowapi_lasttime + CHECK_FLOWAPI_INTERVAL)) {
		...
		#if defined(ARDUINO)
		if (!ui_state) {
			os.lcd_print_line_clear_pgm(PSTR("Check Flow API..."),1);
		}
		#endif
		GetFlowData();
	}
